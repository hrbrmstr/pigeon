        ...
        unsigned move_number, Boolean white_to_move)
{
    ...
    if (GlobalState.keep_NAGs) {
        if(GlobalState.json_format && move_details->Nags != NULL) {
            fputs(", \"nags\" : [", outputfile);
        }
        while (nags != NULL) {
            if(GlobalState.json_format) {
                fprintf(outputfile, "\"%s\"", nags->str);
                if(nags->next != NULL) {
                    fputs(", ", outputfile);
                }
            }
...
//                exit(1);
    ...
    if (GlobalState.add_hashcode_comments) {
        if(GlobalState.json_format) {
            ...
            char *hashcode = malloc_or_die(HASH_64_BIT_SPACE + 1);
...
format_game(Game *current_game, FILE *outputfile)
{
    ...
    Board *initial_board = new_game_board(current_game->tags[FEN_TAG]);
    ...
    if (final_board != NULL) {
        if (GlobalState.output_plycount) {
            char count[FORMATTED_NUMBER_SIZE];
            unsigned ply = 2 * (final_board->move_number - 1);
            if(final_board->to_move == BLACK) {
                ply++;
            }
            sprintf(count, "%u", ply);
            current_game->tags[PLY_COUNT_TAG] = copy_string(count);
        }
...
build_FEN_comment(const Board *board)
{
    ...
    char *comment = malloc_or_die(strlen(prefix) + strlen(fen) + strlen(suffix) + 1);
