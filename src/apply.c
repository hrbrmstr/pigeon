            ...
            if (check_move_validity) {
                if (apply_move(next_move, board)) {
                    ...
                    if (GlobalState.check_for_repetition) {
                        Boolean repetition =
                            update_position_counts(game_details->position_counts, board->weak_hash_value);
...
//                                                    result);
                        ...
                        /* Check for inconsistentcy between Result tag and game result. */
                        const char *move_result = next_move->terminating_result;
                        const char *result_tag = game_details->tags[RESULT_TAG];
                        if(result_tag != NULL &&
                               move_result != NULL &&
                               strcmp(move_result, "*") != 0) {
                            if(strcmp(result_tag, move_result) != 0) {
                                print_error_context(GlobalState.logfile);
                                fprintf(GlobalState.logfile,
...
apply_move_list(Game *game_details, unsigned *plycount, unsigned max_depth)
{
    ...
    if (GlobalState.check_for_repetition && game_details->position_counts == NULL) {
        game_details->position_counts = new_position_count_list(board->weak_hash_value);
    }
...
{ /* Assume success. */
    ...
    if (GlobalState.output_format == SOURCE ||
         ...
         * NB: @@@ We should really check the value of the Variant tag.
         */
        if ((class == KINGSIDE_CASTLE || class == QUEENSIDE_CASTLE) &&
                game != NULL &&
                game->tags[VARIANT_TAG] != NULL &&
                strcmp(game->tags[VARIANT_TAG], "chess 960") == 0) {
                ...
                if(move_details->comment_list != NULL &&
                    ...
                    while(!match && comment != NULL) {
                        ...
                        if(!match) {
                            ...
                            string_list->str = get_FEN_string(board);
                        }
                    }
                }
...
char *get_FEN_string(const Board *board)
{
    char epd[FEN_SPACE], fen_suffix[FEN_SPACE];
    build_FEN_components(board, epd, fen_suffix);
    char *FEN_string = malloc_or_die(strlen(epd) + 1 +
            strlen(fen_suffix) + 1);
