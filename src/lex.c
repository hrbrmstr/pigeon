...
add_filename_list_from_file(FILE *fp, SourceFileType file_type)
{
    ...
    if (list_of_files.files != NULL) free(list_of_files.files);
    if (list_of_files.file_type != NULL) free(list_of_files.file_type);

    list_of_files.files = NULL;
    list_of_files.file_type = NULL;
    list_of_files.max_files = 0;
...
//    printf("%s\n", list_of_files.files[file_number]);
